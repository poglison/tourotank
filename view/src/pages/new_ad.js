            ...
            <Container className="w-full md:w-[calc(100%-326px)] min-h-screen">
                ...
                <div className="w-full mt-4 flex flex-col">
                    ...
                    <div className="flex flex-col">
                        ...
                        <input onChange={(e) => { setAd({ ...ad, title: e.target.value }) }} placeholder="Digite o titulo do anúncio" className="border dark:border-stone-700 p-2 px-4 rounded-lg mt-1 dark:bg-stone-900 outline-none font-ibm text-lg text-stone-800 dark:text-stone-300 dark:placeholder:text-stone-600" />
                    </div>

                    <div className="flex flex-col mt-4 ">
                        <span className="font-ibm text-xs text-stone-600">Descrição</span>
                        <textarea onChange={(e) => { setAd({ ...ad, description: e.target.value }) }} placeholder="Digite a descrição do anúncio" className="border dark:border-stone-700 p-2 px-4 rounded-lg mt-1 dark:bg-stone-900 xl:h-36 xl:overflow-auto font-ibm text-lg text-stone-700 dark:text-stone-400 outline-none resize-none dark:placeholder:text-stone-600" />
                    ...
                    <div className="flex flex-col mt-5">
                        ...
                        <Select isOpen={isOpen} disabled={false} selected={false} setIsOpen={setIsOpen} onChange={(option) => { setAd({ ...ad, category: option }); }} options={categories} />
                    </div>
                    ...
                    <div className="w-full flex flex-col mt-6">
                        ...
                        {type == 1 ?
                            <div className="flex flex-col">
                                ...
                                }} className="border dark:border-stone-700 p-2 px-4 rounded-lg mt-1 dark:bg-stone-900 outline-none font-ibm text-lg text-stone-800 dark:text-stone-300 dark:placeholder:text-stone-600" />
                            </div>
                            :
                            null
                        }
                        ...
                        <div className="flex flex-col mt-5">
                            ...
                            <input onChange={(e) => { setAd({ ...ad, qtd: e.target.value }) }} className="border dark:border-stone-700 p-2 px-4 rounded-lg mt-1 dark:bg-stone-900 outline-none font-ibm text-lg text-stone-800 dark:text-stone-300 dark:placeholder:text-stone-600" />
                        </div>
                        <div className="mt-10 border dark:border-stone-700 h-36 object-cover rounded-lg flex items-center justify-center cursor-pointer hover:bg-stone-50 dark:bg-stone-900 hover:dark:bg-stone-800 text-stone-500 text-lg relative">
                            <input id="file" type="file" multiple="multiple" className="absolute w-full h-full hidden" onChange={(e) => { convertBase64(e) }} />
                            <label for="file" className="cursor-pointer absolute w-full h-full flex items-center justify-center rounded-lg" />
                        ...
                        <div className="flex">
                            ...
                            {files?.length > 0 ?
                                files.map((file, i) => {
                                    return (
                                        <div key={i} className="w-20 h-20 bg-stone-850 rounded-lg mt-4 mr-4 relative">
                                            <img src={file} alt="Arquivo anexado"
                                                className="w-full h-full object-cover rounded-lg"/>
                                            <div onClick={() => { setFiles(files.filter((item, index) => index != i)) }}
                                                className="absolute top-0 right-0 w-6 h-6 bg-stone-850 rounded-full flex items-center justify-center cursor-pointer hover:bg-stone-800">
