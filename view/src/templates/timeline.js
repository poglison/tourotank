...
const TimeLineCircle = (props) => {
    return (
        <div onClick={() => props.setCheck ? props.setCheck(props.index) : null} className={"flex flex-col cursor-pointer items-center justify-center border w-full h-32 rounded " + (props.check ? "border-primary bg-primary/5 hover:bg-primary/10" : "  hover:bg-stone-50 dark:hover:bg-stone-900 dark:border-stone-800")}>
            <div className={"flex items-center justify-center w-10 h-10 rounded-full " + (props.check ? "bg-primary" : "bg-stone-300 border dark:border-stone-800")}>
